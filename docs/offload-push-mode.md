...
> 本文档描述 `RDMA_direct` 分支引入的 **Push 读取模式**,作为 [offload-mechanism.md](offload-mechanism.md) 第 2.2 节「Load（SSD → 请求方）」的扩展。
...
4. **构建/测试**:需要在具备 Mooncake 依赖、RDMA/URMA 与 etcd 环境的 Linux 目标机上做编译验证与端到端压测。
```
