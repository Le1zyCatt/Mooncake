...
TEST(TransportSelectorTest, UbTransportNameRoundTrips) {
    const auto name = TransportSelector::transportTypeName(UB);
    EXPECT_EQ(name, "ub");
    EXPECT_EQ(TransportSelector::parseTransportType(name), UB);
}
...
TEST(TopologyTest, LegacyJsonDefaultsNativeUbIdentityFields) {
    ...
    EXPECT_EQ(nic->native_name, "legacy-nic");
    EXPECT_EQ(nic->device_index, -1);
    EXPECT_EQ(nic->eid_index, -1);
    EXPECT_TRUE(nic->eid.empty());
    EXPECT_TRUE(nic->active);
}

TEST(TopologyTest, UbIdentityFieldsRoundTripThroughJson) {
    Topology source;
    Topology::NicEntry ub;
    ub.name = "ub-device-0/eid-2";
    ub.native_name = "ub-device-0";
    ...
    EXPECT_EQ(round_tripped->native_name, ub.native_name);
    ...
    EXPECT_EQ(round_tripped->device_index, ub.device_index);
    EXPECT_EQ(round_tripped->eid_index, ub.eid_index);
    EXPECT_EQ(round_tripped->eid, ub.eid);
    EXPECT_EQ(round_tripped->active, ub.active);
}
