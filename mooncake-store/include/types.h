...
/**
 * A holder-specific request to tombstone one LOCAL_DISK replica.
 *
 * object_version is generated by the bucket backend when the replica is first
 * persisted and remains stable when that object is moved by compaction.  It is
 * therefore an incarnation identifier, rather than a key version inferred
 * from the current Master entry.  Delayed tasks for a deleted incarnation can
 * safely be ACKed after the same key has been recreated.
 */
struct RemoveTaskItem {
    ...
    bool operator==(const RemoveTaskItem& other) const {
        return task_id == other.task_id && tenant_id == other.tenant_id &&
               key == other.key && object_version == other.object_version &&
               data_size == other.data_size;
    }
};
YLT_REFL(RemoveTaskItem, task_id, tenant_id, key, object_version, data_size);

/** A local SSD entry sent in bounded batches during startup reconciliation. */
struct LocalDiskObjectInfo {
    std::string tenant_id;
    std::string key;
    UUID object_version{0, 0};
    int64_t data_size{0};
};
YLT_REFL(LocalDiskObjectInfo, tenant_id, key, object_version, data_size);
...
struct StorageObjectMetadata {
    ...
    // Stable identity of this local replica.  Bucket compaction must preserve
    // it; a new offload of the same tenant/key receives a new value.
    UUID object_version{0, 0};
